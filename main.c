/*
 * Attempt one for the first assignment of iot 2021
 */
...
/**
 *  Definitions for i/o stuff
 */
    #define ADC_LUX_LINE ADC_LINE(0)
    #define ADC_LUX_RES ADC_RES_12BIT

    #define LUX_CHECK_DELAY (100LU * US_PER_MS) /* 100 ms */
/**
 * 
 */
...
// Start new thread with emcute running forever
...
// What happens when a pub command occur
...
// Command to call when attempting to start a connection
...
// Command to call when attempting to end a connection
...
// Command to call when attempting to PUB something
...
// Command to call when attempting to SUB to a topic
...
// Command to call when attempting to UNSUB from a topic
...
// Command to call when attempting to set a lest will
...
int main(void)
{
    puts("This is My first homework");

    // REVIEW THIS FRAGMENT AND ALL THE FUNCTIONS
    ...
    while (1)
    {
        sample = adc_sample(ADC_LUX_LINE, ADC_LUX_RES);
        lux = adc_util_map(sample, ADC_LUX_RES, 10, 100);
        ...
        {
            printf("ADC_LINE(%u): raw value: %i, lux: %i\n", ADC_LUX_LINE, sample, lux);
        }

        xtimer_periodic_wakeup(&last, LUX_CHECK_DELAY);
    }
